...
export default {
  ...
  computed: {
    ...
    posterTransform() {
      ...
      if (/https?/.test(poster)) {
        return poster;
      } else if (/.(webp|bmp|jpg|png|tif|gif)$/.test(poster)) {
        return vdk.v3platform.getSrcPathFromRes(this, poster);
      } else {
        return vdk.v3platform.getSrcPathFromId2url(this, poster);
      }
    },
    videoType() {
      let src = this.src;
  ...
  methods: {
    init() {
      ...
      let player = (this.videojsEl = videojs(this.widgetCode, opt, function() {
    ...
    handelResolution() {
      ...
      $('.vjs-resolution-button')
        ...
        .bind('click', function(event) {
          ...
          let $show = $(`#${self.containerId}`).find('.vjs-resolution-button-label');
