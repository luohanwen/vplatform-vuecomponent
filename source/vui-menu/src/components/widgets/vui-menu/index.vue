...
export default {
  ...
  render(h) {
    ...
    let getMenuContent = (item) => {
      ...
      if (iconType && (pid === '' || pid === undefined)) {
        return [h('Icon', { attrs: { type: iconType } }), h('span', title)];
      } else {
        return title;
      }
    };
    ...
    return h(
      ...
      [getItemTree(this.realData), h('div', { slot: 'header' }, this.$slots.header), h('div', { slot: 'footer' }, this.$slots.footer)]
    );
  },
  data: function() {
    return {};
  ...
  computed: {
    realData() {
      let dataSource = this.getValidateSource(this.dataSource);
      console.log('dataSource', dataSource);
      let val = [];
      if (dataSource.length) {
        if (this.mode == 'horizontal' && this.fullscreen) {
          val = Utils.toTreeSpecial(dataSource, this.nameField, this.pnameField);
        } else {
          val = Utils.toTree(dataSource, this.nameField, this.pnameField);
        }
      }
      return val;
    }
