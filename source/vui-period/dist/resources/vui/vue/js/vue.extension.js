...
Vue.directive("res", function (el, binding, node) {
	if (binding.value == undefined || binding.value == null || !binding.value || binding.value == "") {
		return;
	}
	var vue = node.context;
	/* 递归去获取构件编码 */
	var getComponentCode = function (node) {
	...
	var componentCode = getComponentCode(vue);
	if (!componentCode) {
...
Vue.directive("id2url", function (el, binding) {
	...
	// 如果是本地预览环境则将url替换为基于本地资源的相对路径
	if (window._$v3platform_preview_env) {
		const preViewResPath = 'itop/entityRes';
		enhanceUrl(el, binding.arg, `${preViewResPath}/${binding.value}`);
	} else {
				...
				return function (func) {
					if (typeof (nowVue._$registerDsEvent) != 'function') {
						if (typeof (func) == 'function')
							nowVue.dsEventRegisterFunc.push(func);
					} else {
					...
					for (var i = 0, len = components.length; i < len; i++) {
						...
						if (dataType == "Array") {
							...
							if (attrName) {
								...
								if (value_field_attr_name != "" && childNode.hasAttribute(value_field_attr_name)) {
