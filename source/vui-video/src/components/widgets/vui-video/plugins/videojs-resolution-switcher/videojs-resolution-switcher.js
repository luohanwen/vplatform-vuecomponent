	...
	(function(window, videojs) {
		...
		var ResolutionMenuItem = videojs.extend(MenuItem, {
			...
			onClick: function(customSourcePicker) {
				//   haven todo 阻止组件本身的点击逻辑，由调用组件自己重新实现点击逻辑
				return false;
						...
						menuItems.push(
							new ResolutionMenuItem(
								this.player_,
								{
									...
									initialySelected:
										key ===
										this.options_.initialySelectedLabel,
									customSourcePicker: this.options_
										.customSourcePicker
								},
		...
		videoJsResolutionSwitcher = function(options) {
			...
			player.updateSrc = function(src) {
				...
				player.controlBar.resolutionSwitcher = player.controlBar.el_.insertBefore(
					menuButton.el_,
					player.controlBar.getChild("volumePanel").el_
				);
				player.controlBar.resolutionSwitcher.dispose = function() {
					this.parentNode.removeChild(this);
				};
				return setSourcesSanitized(
					player,
					choosen.sources,
					choosen.label
				);
			...
			function initResolutionForYt(player) {
				...
				player.one("play", function() {
					...
					var _customSourcePicker = function(
						_player,
						_sources,
						_label
					) {
						player.tech_.ytPlayer.setPlaybackQuality(
							_sources[0]._yt
						);
						return player;
					};
